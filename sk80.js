var SK80 = (function (undef) {
    ...
    function identifyObject(o) {
        ...
        if (str !== undef) {
            str = str.split(' ').pop();
            str = toLower.call(str.substring(0, str.length - 1));
        }
        return str;
    }
    ...
    // on whether that key can be treated.
    SK80.keys = (function () {

        var hasObjKeys = hasOwn.call(Object, 'keys');
        ...
        return function (key) {
            ...
            if (hasObjKeys) {
                keys = Object.keys(treatments);
            } else {
                for (i in treatments) {
                    if (hasOwn.call(treatments, i)) {
                        keys.push(i);
                    }
                }
            }

            // Now we have our array, either return it or check it.
            return key === undef ? keys : indexOf.call(keys,
                    identifyObject(key)) > -1;
    ...
    SK80.reduce = function (obj) {
        ...
        for (key in obj) {
            if (hasOwn.call(obj, key) && key.charAt(0) === '$') {
